...
/**
 * app/(dashboard)/inventario/registros/page.tsx
 * -------------------------------------------------------
 * Historial de Movimientos de Inventario (Ledger de Auditoría).
 * 
 * Muestra el registro cronológico inmutable de todas las
 * entradas y salidas de stock:
 * - Entradas por compras / imprenta / ajustes
 * - Entradas por devoluciones de muestras escolares (promociones)
 * - Entradas por devoluciones de consignaciones
 * - Salidas por ventas directas
 * - Salidas por muestras a colegios (promociones)
 * - Salidas por entregas a vendedores (consignaciones)
 * -------------------------------------------------------
 */
...
export default function RegistrosPage({ searchParams }: RegistrosPageProps) {
  const resolvedSearchParams = searchParams ? use(searchParams) : undefined
  const tipoInicial = (resolvedSearchParams?.tipo as 'entrada' | 'salida') || 'todos'
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Los movimientos se generan automáticamente al vender, entregar muestras, o al cargar stock.
            </p>
          </CardContent>
        </Card>
      ) : (
        ...
        <div className="space-y-4">
            ...
            >
              {productos.map((p) => (
                <option key={p.id} value={p.id}>
                  {p.codigo_sku} — {p.nombre} (Stock actual: {p.stock})
                </option>
              ))}
            </select>
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ej: Lote recibido de imprenta, ajuste de stock, etc."
              className="w-full rounded-xl border border-border bg-background px-3 py-2 text-sm focus:border-primary-accent/50 focus:outline-none"
            />
          </div>
